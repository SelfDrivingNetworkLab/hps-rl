...
class DDPGActor(nn.Module):
    def __init__(self, obs_size, act_size, n_hidden = 1, feature_list = [400,300]):
        super(DDPGActor, self).__init__()

        modules = []
        modules.append(nn.Linear(obs_size,feature_list[0]))
        modules.append(nn.ReLU())
        for i in range(n_hidden):
            modules.append(nn.Linear(feature_list[i],feature_list[i+1]))
            modules.append(nn.ReLU())
        modules.append(nn.Linear(feature_list[n_hidden],act_size))
